 ...
 */
const adaptCompetency = (competency: ICommonLibWithSkillType): ICompetitionOption => ({
  key: competency.id,
...
const makeInputValue = (inputValue: string[], delimeter = INPUT_TAG_DELIMETER) =>
 ...
 */
const isOptionMatch = (option: ICompetitionOption | OptionShape, inputValue: string) => {
  if (typeof inputValue !== 'string' || inputValue === '' || typeof option.content !== 'string') {
    return false;
  }
...
const getCompetitionOptionName = (competenceOption: OptionCompetitionShape) =>
  (competenceOption.value?.name as string) || (competenceOption.content as string) || '';
...
const isValidInputValue = (pattern: RegExp, inputValue: string) => pattern.test(inputValue);
